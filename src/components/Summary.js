...
export function Summary({items = [], onOrderSubmit}) {
  ...
  const handleEmailInputChanged = (e) => setOrder(
      {...order, email: e.target.value})
  const handleAddressInputChanged = (e) => setOrder(
      {...order, address: e.target.value})
  const handlePostCodeInputChanged = (e) => setOrder(
  ...
  return (
      <>
        ...
        <hr/>
        {items.map(
            v => <SummaryItem key={v.productId} productName={v.productName}
                              count={v.count}/>
        )}
        <form>
          <div className="mb-3">
            ...
            <input type="email" className="form-control mb-1"
                   value={order.email} onChange={handleEmailInputChanged}
                   id="email"/>
          </div>
          <div className="mb-3">
            <label htmlFor="address" className="form-label">주소</label>
            <input type="text" className="form-control mb-1"
                   value={order.address} onChange={handleAddressInputChanged}
                   id="address"/>
          </div>
          <div className="mb-3">
        ...
        <button className="btn btn-dark col-12" onClick={handleSummit}>결제하기
        </button>
      </>
  )
}